...
define("EVENT_ESPRESSO_REQUIREMENTS_PLUGINPATH", "/" . plugin_basename(dirname(__FILE__)) . "/");
define('EVENT_ESPRESSO_REQUIREMENTS_WPPLUGINPATH', plugin_basename(__FILE__));
define("EVENT_ESPRESSO_REQUIREMENTS_PLUGINFULLPATH", WP_PLUGIN_DIR . EVENT_ESPRESSO_REQUIREMENTS_WPPLUGINPATH);
define("EVENT_ESPRESSO_REQUIREMENTS_PHPINFO_FULLURL", plugins_url('assets/phpinfo.php', __FILE__));

function EE_set_session_key_for_logged_in_admin() {
	if (session_status() != PHP_SESSION_ACTIVE)
		session_start();
	if (current_user_can('activate_plugins'))
		$_SESSION['EE_REQUIREMENTS_IS_ADMIN'] = TRUE;
}

add_action('init', 'EE_set_session_key_for_logged_in_admin');

function EE_clear_session_key_on_logout() {
	if (session_status() != PHP_SESSION_ACTIVE)
		session_start();
	if (current_user_can('activate_plugins'))
		$_SESSION['EE_REQUIREMENTS_IS_ADMIN'] = FALSE;
}

add_action('wp_logout', 'EE_clear_session_key_on_logout');
...
$is_mysql_valid = version_compare($mysql_version, '5.0.0', '>'); // checks the mysql version
...
$has_gd_php_module = !function_exists('imagecreatefromgif') ? false : true;
$has_recommended_mysql = version_compare($mysql_version, '5.5.0', '>'); // mysql 5.5 is recommended
$has_recommended_wordpress = version_compare($wp_version, '3.7', '>='); // the current version of wordpress is recommended
...
if (strpos($error_level, 'E_STRICT')) {
	$is_php_strict = true;
}
if (WP_DEBUG == true) {
	$is_wp_debug = true;
...
if ($is_wp_valid) { // fixes fatal error if wp_get_theme doesn't exist, this was added in 3.4 so if they've already failed their WordPress version check, this will fail.
	...
	if (strpos($theme_author_uri, 'themeforest') || strpos($theme_style, 'themeforest')) {
		$is_themeforest = true;
	} // is the theme a themeforest theme?
} else {
	if (strpos($theme_style, 'themeforest')) {
		$is_themeforest = true;
	} // if we can't run wp_get_theme, still check if it's a themeforest theme
	if ($is_themeforest) {
...
if (strpos($dns, 'mediatemple') || strpos($dns, 'MEDIATEMPLE')) {
	$is_mediatemple = true;
} // if the nameserver contains mediatemple, this is a mediatemple site
if (strpos($dns, 'domaincontrol') || strpos($dns, 'DOMAINCONTROL')) {
	$is_godaddy = true;
} // if the nameserver contains domaincontrol, this is a godaddy site
if (strpos($dns, 'wpengine')) {
	$is_wpengine = true;
	$is_godaddy = false;
} // if there's a record to point to wpengine, we're actually using their servers, not GoDaddy's, so GoDaddy is always false
if (strpos($webserver, 'pache')) {
	$is_apache = true;
} // checks if the server is apache
if (strpos($webserver, 'IS')) {
	$is_iis = true;
} // checks the _SERVER variable for IIS
if (function_exists('apache_get_modules')) {
	$apache_mods = true;
...
// pass or fail
if (!$is_curl_valid || $is_iis || $is_godaddy || !$is_php_mem_ok || !$has_recommended_wordpress || !$has_mod_security || !$has_max_upload) {
	$is_warn = true;
} // any warnings?
if ($is_php_valid && $is_mysql_valid && $is_wp_valid) {
	$is_pass = true;
} else {
	$is_fail = true;
} // did we pass?
...
function espresso_requirements_page() {
	...
	<div class="row-fluid">
		<div class="span2"><?php _e('PHP allowed to open files over url protocol', 'espresso-requirements'); ?></div>
		<div class="span10"><?php echo ini_get('allow_url_fopen') ? __('Yes', 'event_espresso') : __('No', 'event_espresso'); ?></div>
	</div>
	<div class="row-fluid">
		<div class="span2"><?php _e('PHP equipped with GD image manipulation library', 'espresso-requirements'); ?></div>
		<div class="span10"><?php echo $has_gd_php_module ? __('Yes', 'event_espresso') : __('No', 'event_espresso'); ?></div>
	</div>
	<div class="row-fluid">
		<div class="span2"><?php _e('PHP Info', 'espresso-requirements'); ?></div>
		<div class="span1"></div>
		<div class="span9 alert info"><i class="icon-exclamation-sign"></i> <a class="thickbox" href="<?php
	echo add_query_arg(array(
		'KeepThis' => true,
		'TB_iframe' => true,
		'height' => 600,
	...
	<div class="row-fluid">
		<div class="span2"><?php _e('Test actual PHP memory limit', 'espresso-requirements'); ?></div>
		<div class="span1"><form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>"><input class="button-primary" type="submit" name="Submit" value="Test"></form></div>
	<?php if (!empty($_POST['Submit']) && $_POST['Submit'] == "Test") { ?>
			<div class="span9 alert info">
				<form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>"><input class="button-primary" type="submit" name="Submit" value="Reload Without Test"></form>
			</div>
	<?php
	}
	if (!empty($_POST['Submit']) && $_POST['Submit'] == "Test") {
		...
		for ($i = 1; $i < 100; $i++) {
			$a = loadmem($i);
			echo "You have successfully allocated " . memory_get_usage(true) / (1024 * 1024) . "MB of memory" . "<br />";
			unset($a);
		}
	}
	?>
	...
	<section>
		<div class="row-fluid">
			<div class="span2"><?php _e('Your version', 'espresso-requirements'); ?></div>
			<div class="span1 break-word "><?php echo $mysql_version; ?></div>
